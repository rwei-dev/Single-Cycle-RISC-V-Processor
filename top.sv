module top(input logic MAX10_CLK1_50,
  ...
  logic [31:0] PC, Instr, ReadData, ReadDataMux;
  logic clk;
  ...
  assign HEX0 = ~HEX3HEX0[7:0];
  assign HEX1 = ~HEX3HEX0[15:8];
  assign HEX2 = ~HEX3HEX0[23:16];
  assign HEX3 = ~HEX3HEX0[31:24];

  assign HEX4 = ~HEX5HEX4[7:0];
  assign HEX5 = ~HEX5HEX4[15:8];
  
  // instantiate processor and memories
  riscvsingle rvsingle(clk, ~KEY[0], PC, Instr, MemWrite, DataAdr, WriteData, ReadDataMux);
  imem imem(PC, Instr);
  dmem dmem(clk, MemWrite, DataAdr, WriteData, ReadData);
  
  always_comb begin
    if (DataAdr === 32'hFF200040)
      ReadDataMux = {22'b0, SW[9:0]};
    else
      ReadDataMux = ReadData;
  end
  ...
  always_ff @(posedge clk) begin
    if (~KEY[0]) begin
      ...
      if (DataAdr === 32'hFF200000)
        LEDR <= WriteData[9:0];
      if (DataAdr === 32'hFF200020)
        HEX3HEX0 <= WriteData[31:0];
      if (DataAdr ===  32'hFF200030)
...
module regfile(input logic clk,
  ...
  // read two ports combinationally (A1/RD2, A2, RD2)
...
module maindec(input logic [6:0] op,
  ...
  always_comb
    case(op)
      ...
      default: controls = 11'bx_xx_x_x_xx_x_xx_x; // improper instruction
    endcase
endmodule
